...
import {ArrowRight, CalendarRange, MapPinned, ShieldCheck} from 'lucide-react';
            ...
            <div className="min-w-0 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                <div className="rounded-2xl border border-white/70 bg-white/75 p-3 backdrop-blur sm:p-4 dark:border-white/10 dark:bg-white/5">
                  <p className="text-xl font-semibold sm:text-2xl">{cars.length}</p>
                  <p className="text-xs text-muted-foreground sm:text-sm">
                    {t('stats.carsShown')}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white/75 p-3 backdrop-blur sm:p-4 dark:border-white/10 dark:bg-white/5">
                  <p className="text-xl font-semibold sm:text-2xl">{t('stats.live')}</p>
                  <p className="text-xs text-muted-foreground sm:text-sm">
                    {displayedFleetLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white/75 p-3 backdrop-blur sm:p-4 dark:border-white/10 dark:bg-white/5">
                  <p className="text-xl font-semibold sm:text-2xl">{t('stats.fast')}</p>
                  <p className="text-xs text-muted-foreground sm:text-sm">
        ...
        <section id="available-cars">
          ...
          {cars.length === 0 ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {cars.map((car) => (
                <CarCard key={car.id} car={car} bookable={car.status === 'AVAILABLE'} />
              ))}
            </div>
          )}
        </section>
